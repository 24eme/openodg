...
<?php if (count($produit->getProduitsCepage()) > 0): ?>
    <tr>
        <td class="hiddenRow" colspan="<?php if($drev->hasProduitsVCI() || $drev->declaration->hasVolumeRevendiqueVci()): ?>5<?php else: ?>4<?php endif; ?>"  >
                        ...
                        <tbody>
                            ...
                            foreach ($produit->getProduitsCepage() as $cepage_key => $produit_cepage) :
                                ...
                                <?php if ($produit_cepage->volume_revendique_vt && $vtsgn): ?>
                                    <tr>
                                        <td class="col-md-<?php if($drev->hasProduitsVCI() || $drev->declaration->hasVolumeRevendiqueVci()): ?>4<?php else: ?>6<?php endif; ?> text-muted revendication_recap_td_libelle"><small><?php echo $produit_cepage->getLibelle() ?> - VT</small></td>
                                        <?php if(!$drev->isNonRecoltant()): ?>
                                        <td class="text-center text-muted col-md-2">
                                ...
                                <?php if ($produit_cepage->volume_revendique_sgn && $vtsgn): ?>
                                    <tr>
                                        <td class="col-md-<?php if($drev->hasProduitsVCI() || $drev->declaration->hasVolumeRevendiqueVci()): ?>4<?php else: ?>6<?php endif; ?> text-muted revendication_recap_td_libelle"><small><?php echo $produit_cepage->getLibelle() ?> - SGN</small></td>
                                        <?php if(!$drev->isNonRecoltant()): ?>
                                        <td class="text-center text-muted col-md-2">
                                ...
                                <?php if (!$print): ?>
                                    <tr>
                                        <td class="col-md-<?php if($drev->hasProduitsVCI() || $drev->declaration->hasVolumeRevendiqueVci()): ?>4<?php else: ?>6<?php endif; ?> text-muted revendication_recap_td_libelle"><small><?php echo $produit_cepage->getLibelle() ?></small></td>
                                        <?php if(!$drev->isNonRecoltant()): ?>
                                        <td class="text-center text-muted col-md-2"></td>
                                        <?php endif; ?>
                                        <?php if ($drev->canHaveSuperficieVinifiee()): ?>
