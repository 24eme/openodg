...
class ConditionnementConfiguration {
    ...
    public static function getInstance() {
        if (is_null(self::$_instance)) {
            self::$_instance = new ConditionnementConfiguration();
        }
        return self::$_instance;
    }

    public function load() {
        $this->configuration = sfConfig::get('cond_configuration_conditionnement', array());
    }

    public function __construct() {
        if(!sfConfig::has('cond_configuration_conditionnement')) {
			throw new sfException("La configuration pour les conditionnement n'a pas été défini pour cette application");
		}
