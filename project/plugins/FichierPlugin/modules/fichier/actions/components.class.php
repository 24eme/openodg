...
class fichierComponents extends sfComponents {
    ...
    public function executeMonEspace(sfWebRequest $request) {
        ...
        if(!$this->dr && class_exists("SV11Client")) {
    	    $this->sv = SV11Client::getInstance()->findByArgs($this->etablissement->identifiant, $this->campagne);
        }
        if(!$this->sv && class_exists("SV12Client")) {
    	    $this->sv = SV12Client::getInstance()->findByArgs($this->etablissement->identifiant, $this->campagne);
        }
    }
