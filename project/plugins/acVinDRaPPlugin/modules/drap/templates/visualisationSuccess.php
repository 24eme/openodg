...
<div class="row row-margin row-button">
    ...
    <div class="col-xs-2 text-center">
            <a href="<?php echo url_for('drap_export_pdf', $parcellaireIrrigable) ?>" class="btn btn-success">
    ...
    <div class="col-xs-2 text-right">
        <?php if ($parcellaireIrrigable->validation && ParcellaireSecurity::getInstance($sf_user, $parcellaireIrrigable->getRawValue())->isAuthorized(ParcellaireSecurity::DEVALIDATION)): ?>
                    <a class="btn btn-xs btn-default pull-right" href="<?php echo url_for('drap_devalidation', $parcellaireIrrigable) ?>" onclick="return confirm('Êtes-vous sûr de vouloir dévalider votre déclaration ?');"><span class="glyphicon glyphicon-remove-sign"></span>&nbsp;&nbsp;Dévalider</a>
        <?php endif; ?>
    </div>
    <div class="col-xs-3 text-right">
        ...
        <?php elseif(!$parcellaireIrrigable->validation_odg && $sf_user->isAdmin()): ?>
                <a onclick='return confirm("Êtes vous sûr de vouloir approuver cette déclaration ?");' href="<?php echo url_for("drap_validation_admin", array("sf_subject" => $parcellaireIrrigable, "service" => null)) ?>" class="btn btn-default btn-upper"><span class="glyphicon glyphicon-ok-sign"></span>&nbsp;&nbsp;Approuver</a>
        <?php endif; ?>
    </div>
</div>
