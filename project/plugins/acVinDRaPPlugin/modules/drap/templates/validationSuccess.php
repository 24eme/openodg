...
<form role="form" action="<?php echo url_for('drap_validation', $drap) ?>" method="post" id="validation-form">
    ...
    <div style="padding-top: 10px;" class="row row-margin row-button">
        <div class="col-xs-4">
            <a href="<?php echo url_for("drap_destinations", $drap) ?>" class="btn btn-default btn-upper"><span class="glyphicon glyphicon-chevron-left"></span> Retourner à l'étape précédente</a>
        </div>
        ...
        <div class="col-xs-4 text-right">
            <button type="button" id="btn-validation-document" data-toggle="modal" data-target="#drap-confirmation-validation" <?php if (isset($validation) && $validation->hasErreurs()): ?>disabled="disabled"<?php endif; ?> class="btn btn-success btn-upper"><span class="glyphicon glyphicon-check"></span>&nbsp;&nbsp;Valider votre déclaration</button>
        </div>
    </div>
    <?php if (!isset($validation) || !$validation->hasErreurs()): ?>
	<?php include_partial('drap/popupConfirmationValidation', array('form' => $form)); ?>
	<?php endif; ?>
</form>
