...
class DRevValidation extends DeclarationLotsValidation
{
    ...
    protected function controleVolumeMediterraneeRoseDeclare(){
        ...
        $produit = $this->document->declaration->get(VIP2C::getProduitHashWithVolumeSeuil())->getConfig()->getLibelleComplet();

        $volumeTotalMediterraneeRoseDeclare = $this->document->summerizeProduitsLotsByCouleur()[$produit]['volume_lots'];
        $volumeCommercialisableLibre = $this->document->getVolumeCommercialisableLibre(VIP2C::getProduitHashWithVolumeSeuil());
        $volumeMaxAutorise = $this->document->getVolumeRevendiqueSeuil(VIP2C::getProduitHashWithVolumeSeuil());
