<div class="col-sm-6 col-md-4 col-xs-12">
    <div class="block_declaration panel <?php if ($intentionCremant && $intentionCremant->validation): ?>panel-success<?php else: ?>panel-primary<?php endif; ?>">
        ...
        <?php elseif (!ParcellaireClient::getInstance()->isOpen(ParcellaireClient::TYPE_COUCHDB_INTENTION_CREMANT)): ?>
            <div class="panel-body">
                <?php if(date('Y-m-d') > ParcellaireClient::getInstance()->getDateOuvertureFin(ParcellaireClient::TYPE_COUCHDB_INTENTION_CREMANT)): ?>
                <p>Le Téléservice est fermé. Pour toute question, veuillez contacter directement l'AVA.</p>
                <?php else: ?>
                <p>Le Téléservice sera ouvert à partir du <?php echo format_date(ParcellaireClient::getInstance()->getDateOuvertureDebut(ParcellaireClient::TYPE_COUCHDB_INTENTION_CREMANT), "D", "fr_FR") ?>.</p>
                <?php endif; ?>
            </div>
            <div class="panel-bottom">
                <?php if ($sf_user->isAdmin()): ?>
