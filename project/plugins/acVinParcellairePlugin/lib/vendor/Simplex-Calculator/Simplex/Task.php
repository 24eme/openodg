...
final class Task
{
	...
	public function addRestriction(Restriction $r)
	{
		if ($r->getVariableList() !== $this->function->getVariableList()) {
			throw new \InvalidArgumentException("Restriction variables don't match the objective function variables. (".implode(',',$r->getVariableList())." !== ".implode(',',$this->function->getVariableList()).")");
		}
