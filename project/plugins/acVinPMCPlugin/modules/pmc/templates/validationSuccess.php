...
<form role="form" class="form-horizontal" action="<?php echo url_for('pmc_validation', $pmc) ?>#engagements" method="post" id="validation-form">
    ...
    <div class="row row-margin row-button">
        ...
        <div class="col-xs-6 text-right">
            <button type="button" id="btn-validation-document" data-target="#pmc-confirmation-validation" <?php if($validation->hasErreurs() && $pmc->isTeledeclare() && (!$sf_user->hasPMCAdmin() || $validation->hasFatales())): ?>disabled="disabled"<?php endif; ?> class="btn btn-success btn-upper "><span class="glyphicon glyphicon-check"></span>&nbsp;&nbsp;Valider la déclaration</button>
        </div>
    </div>
</form>
