...
<div class="col-sm-6 col-md-4 col-xs-12">
    <div class="block_declaration panel <?php if ($travauxmarc && $travauxmarc->validation): ?>panel-success<?php else: ?>panel-primary<?php endif; ?>">
        ...
        <?php if ($travauxmarc && $travauxmarc->validation): ?>
            ...
            <div class="panel-bottom-documents <?php if (DRevSecurity::getInstance($sf_user, $travauxmarc->getRawValue())->isAuthorized(DRevMarcSecurity::DEVALIDATION)): ?>panel-bottom-documents-etape <?php endif; ?>">
                <a href="<?php echo url_for('pieces_historique', array('sf_subject' => $etablissement, 'categorie' => 'travauxmarc')) ?>" class="btn btn-xs btn-link btn-block">Voir tous les documents</a>
            </div>
        ...
        <?php elseif (!TravauxMarcConfiguration::getInstance()->isOpen()): ?>
            <div class="panel-body">
                <?php if(date('Y-m-d') > TravauxMarcConfiguration::getInstance()->getDateOuvertureFin()): ?>
                <p>Le Téléservice est fermé. Pour toute question, veuillez contacter directement l'AVA.</p>
                <?php else: ?>
                <p>Le Téléservice sera ouvert à partir du <?php echo format_date(TravauxMarcConfiguration::getInstance()->getDateOuvertureDebut(), "D", "fr_FR") ?>.</p>
                <?php endif; ?>
            </div>
            <div class="panel-bottom">
