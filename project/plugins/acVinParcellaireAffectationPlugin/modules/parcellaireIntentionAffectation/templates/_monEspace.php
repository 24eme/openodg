...
<div class="col-sm-6 col-md-4 col-xs-12">
    <div class="block_declaration panel  <?php if ($parcellaireAffectation && $parcellaireAffectation->validation): ?>panel-success<?php else : ?>panel-primary<?php endif; ?>">
        ...
        <?php elseif ($parcellaireAffectation):  ?>
          ...
          <?php elseif (!ParcellaireIntentionAffectationClient::getInstance()->isOpen()): ?>
                <div class="panel-body">
                    <?php if(date('Y-m-d') > ParcellaireIntentionAffectationClient::getInstance()->getDateOuvertureFin()): ?>
                    <p>Le Téléservice est fermé. Pour toute question, veuillez contacter directement l'ODG.</p>
                    <?php else: ?>
                    <p>Le Téléservice sera ouvert à partir du <?php echo format_date(ParcellaireIntentionAffectationClient::getInstance()->getDateOuvertureDebut(), "D", "fr_FR") ?>.</p>
                    <?php endif; ?>
                    <div style="margin-top: 50px;">
                        <?php if ($sf_user->isAdmin()): ?>
            ...
            <div class="panel-body">
            	...
            	<div style="margin-top: 50px;">
                    <a class="btn btn-block btn-default" href="<?php echo url_for('parcellaireintentionaffectation_create', array('sf_subject' => $etablissement, 'campagne' => $campagne)) ?>">Démarrer la télédéclaration</a>
                    <?php if ($sf_user->isAdmin()): ?>
                    <?php endif; ?>
                </div>
            </div>
