<div class="col-sm-6 col-md-4 col-xs-12">
    <div class="block_declaration panel <?php if ($intentionCremant && $intentionCremant->validation): ?>panel-success<?php else: ?>panel-primary<?php endif; ?>">
        ...
        <div class="panel-bottom">
                <p>
                    <a class="btn btn-lg btn-block btn-primary" href="<?php echo url_for('parcellaire_affectation_visualisation', $intentionCremant) ?>">Visualiser</a>
                </p>
                <?php if($sf_user->isAdmin()): ?>
                <p>
                    <a onclick='return confirm("Êtes vous sûr de vouloir dévalider cette déclaration ?");' class="btn btn-xs btn-warning pull-right" href="<?php echo url_for('parcellaire_affectation_devalidation', $intentionCremant) ?>"><span class="glyphicon glyphicon-remove-sign"></span>&nbsp;&nbsp;Dévalider la déclaration</a>
                </p>
                <?php endif; ?>
        </div>
        ...
        <div class="panel-bottom">
            <p>
                <a class="btn btn-lg btn-block btn-default" href="<?php echo url_for('parcellaire_affectation_edit', $intentionCremant) ?>"><?php if($intentionCremant->isPapier()): ?><span class="glyphicon glyphicon-file"></span> Continuer la saisie papier<?php else: ?>Continuer la télédéclaration<?php endif; ?></a>
            </p>
            <p>
                <a onclick='return confirm("Êtes vous sûr de vouloir supprimer cette saisie ?");' class="btn btn-xs btn-danger pull-right" href="<?php echo url_for('parcellaire_affectation_delete', $intentionCremant) ?>"><span class="glyphicon glyphicon-trash"></span>&nbsp;&nbsp;Supprimer le brouillon</a>
            </p>
        </div>
