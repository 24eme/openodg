...
foreach ($facets as $type => $ftype) {
  ...
  foreach ($ftype['buckets'] as $f) {
    ...
    echo '<a class="list-group-item list-group-item-xs" href="'.url_for('compte_groupe', array('groupeName' => $f['key'])).'">'.str_replace('_', ' ', $f['key']).'<span class="badge" style="position: absolute; right: 10px;">'.$f['doc_count'].'</span></a>';
    //echo "<a href=''>".$f['key']."</a>";
    if($cpt > 10){
      echo "</div>";
    }
