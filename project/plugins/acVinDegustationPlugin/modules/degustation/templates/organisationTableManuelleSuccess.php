        ...
        <table class="table table-bordered table-striped table_lots text-center">
          ...
          <tbody>
            <?php foreach ($form->getTableLots() as $lot): ?>
                <tr class="hamzastyle-item<?= ($lot->leurre) ? ' warning' : '' ?>" data-words='<?php echo json_encode([$lot->produit_libelle, $lot->numero_dossier, $lot->numero_logement_operateur, $lot->declarant_nom], JSON_HEX_APOS | JSON_HEX_QUOT | JSON_UNESCAPED_UNICODE); ?>'>
