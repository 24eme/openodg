...
\def\NUMFACTURE{<?php echo sprintf("%06d",$facture->numero_ava); ?>}
...
\def\FACTUREDATE{Cotisation <?php echo $date->format("d/m/Y"); ?>}
...
\def\SIRET{783 041 106 00015}
\def\BANQUENOM{CREDIT AGRICOLE PACA}
\def\BANQUEADRESSE{AGENCE LES ARCS}
\def\RIB{FR76 1910 6000 2100 0117 9090 921}
\def\BIC{AGRIFRPP891}
...
\begin{tabular}{|>{\raggedright}m{7.0cm}|r|r|>{\raggedleft}m{2.8cm}|}
  ...
  <?php foreach ($facture->lignes as $ligne): ?>
   \rule[7pt]{0pt}{11pt} <?php echo $ligne->libelle; ?> & & & \tabularnewline
  	<?php foreach ($ligne->details as $detail): ?>
  	    \small{\textit{<?php echo $detail->libelle; ?>}} & \small{\textbf{<?php echo formatQuantite($detail->quantite); ?>}} & \small{\textbf{<?php echo formatFloat($detail->prix_unitaire); ?>}} & \small{\textbf{<?php echo formatFloat($detail->montant_ht); ?>~€}}  \tabularnewline
  	<?php endforeach; ?>
  <?php endforeach; ?>
	~&~&~& \\
	~&~&~& \\
...
\rule[-5pt]{0pt}{18pt} \tabularnewline
  <?php foreach ($facture->lignes as $ligne): ?>
		<?php foreach ($ligne->details as $detail): ?>
			~ & \textbf{TVA\up{*}} \rule[-5pt]{0pt}{18pt} & \textbf{<?php echo formatFloat($detail->montant_tva); ?>~€}
	<?php endforeach; ?>
<?php endforeach; ?>
\rule[-5pt]{0pt}{18pt} \tabularnewline

    	  	  	    & \textbf{TOTAL A PAYER} \rule[-5pt]{0pt}{18pt} & {\textbf{4838.75~€}} \rule[-5pt]{0pt}{18pt} \tabularnewline
  \cline{2-3}
\end{tabular}
\end{center}
	\vspace{1mm}
