...
<ol class="breadcrumb-steps<?php if($stepNum == 5) {echo ' breadcrumb-steps-last';} ?>">
    ...
    <li class="<?php if($stepNum == 3): ?>active<?php endif; ?> <?php if($etapes->isGt($etapeCourante, ParcellaireEtapes::ETAPE_PARCELLES)): ?>visited<?php endif; ?>">
        <div class="step">
            ...
            <a href="<?php echo url_for("parcellaire_parcelles", array('sf_subject' => $parcellaire, 'appellation' => ParcellaireClient::getInstance()->getFirstAppellation($parcellaire->isParcellaireCremant()))) ?>" class="<?php if($stepNum <= 2): ?>ajax<?php endif; ?>">Parcelles</a>
            <?php else: ?>
            <span>Parcelles</span>
            <?php endif; ?>
        </div>
