<?php include_partial('degustation/headerCourrier', ['courrier' => $courrier, "objet" => "Avis de conformité controle vin"]) ?>

<p>Le lot <strong><?php echo showProduitCepagesLot($lot, false, null) ?> de <?php if ($lot->exist('quantite') && $lot->quantite) : ?><?php echo $lot->exist('quantite') ? $lot->quantite : 0 ?> cols<?php else: ?><?php echoFloatFr($lot->volume*1) ?> hl<?php endif; ?> (échantillon n°<?php echo $lot->numero_archive ?>)</strong> a été prélevé pour un examen analytique et organoleptique. Celui-ci n'a relevé aucun manquement au cahier des charges de l'Appellation revendiquée.</p>
