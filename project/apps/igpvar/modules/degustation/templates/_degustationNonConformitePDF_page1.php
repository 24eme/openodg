...
<table>
  ...
  <tr><td>
    <ul>
      ...
      <li>Soit de vous opposer aux conclusions de ce contrôle. Dans ce cas, ainsi que le prévoit le plan de contrôle de l'IGP, vous êtes dans l'obligation de transmettre le dossier à l'organisme de contrôle/d'inspection qui diligentera un nouveau contrôle entièrement à votre charge.
        <?php if (strpos($lot->produit_hash, '/MED/') !== false && Organisme::getInstance(null, 'degustation')->getOiMed()): ?>
            <strong>(<?php echo Organisme::getInstance(null, 'degustation')->getOiMed() ?>)</strong>
        <?php elseif (Organisme::getInstance(null, 'degustation')->getOi()): ?>
            <strong>(<?php echo Organisme::getInstance(null, 'degustation')->getOi() ?>)</strong>
        <?php endif ?></li>
      <?php else: ?>
