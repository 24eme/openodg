...
<?= count($lotsNonConformes) ?><?php if(count($lotsNonConformes) == 1): ?> lot est NON <?php echo $lotsNonConformes[0]->isLibelleAcceptable() ? 'ACCEPTABLE EN L\'ÉTAT' : 'CONFORME'; ?><?php else: ?> lots sont NON <?php echo $lotsNonConformes[0]->isLibelleAcceptable() ? 'ACCEPTABLES' : 'CONFORMES'; ?><?php endif; ?><?php if ($c = count(array_filter($lotsNonConformes->getRawValue(), function (Lot $l) { return $l->conformite !== Lot::CONFORMITE_NONCONFORME_MINEUR; }))): ?>, dont <?php echo $c ?> <?php echo $c > 1 ? 'lots BLOQUÉS' : 'lot BLOQUÉ' ?> à la commercialisation car ayant une non conformité MAJEURE.<?php endif ?>


Vous pouvez télécharger <?php if(count($lotsNonConformes) > 1): ?>les courriers de chacun de ces lots<?php else: ?>le courrier de ce lot<?php endif ?> :
