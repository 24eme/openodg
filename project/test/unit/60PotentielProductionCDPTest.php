...
$t->is_deeply($cepages['principaux'], $principaux, "Cépages principaux respectés.");
$t->is_deeply($cepages['secondaires'], $secondaires, "Cépages secondaires respectés.");

// Test 1 : regle des 2 cepages principaux min
$regle1 = $generator->regleNbCepageMin_GetRevendicable($superficies['principaux'], 2);
$t->is($regle1, $superficies['principaux']['TOTAL'], "2 au moins des cépages principaux sont présents dans l'encépagement.");
...
$t->is($regle2, $superficies['principaux']['TOTAL'], "la proportion d'un des cépages principaux n'est pas supérieure à 90% de l'encépagement");
...
$t->ok($regle2 >= $test3, "La proportion de l'ensemble des cépages principaux est supérieure ou égale à 70% de l'encépagement");

// Test 4 : regle des 30% cepages secondaires
$test4 = round(($regle2+$superficies['secondaires']['TOTAL']) * 0.3, 4);
$t->ok($regle3 <= $test4, "La proportion de l'ensemble des cépages secondaires est inférieure ou égale à 30% de l'encépagement");
...
$t->ok($revendicableSecondairesTotalBlancs <= $test5, "La proportion de l'ensemble des cépages secondaires blancs est inférieure ou égale à 20% de l'encépagement");

// Test 6 : regle des 10% des autres blancs max
$test6 = round(($regle2+$regle3) * 0.1, 4);
$t->ok($revendicableSecondairesAutresBlancs <= $test6, "La proportion de l'ensemble des cépages secondaires autres blancs est inférieure ou égale à 10% de l'encépagement");
...
$t->is_deeply($revendicables, $result, "Calcul du potentiel de production OK.");

// Test 8 : comparaison avec la calculette
$result = ['principaux' => 118.1768, 'secondairesnoirs' => 9.1592, 'secondairesblancs' => 19.0955, 'secondairesvermentinob' => 12.857, 'secondairesautresblancs' => 6.2385];
$t->is_deeply($revendicables, $result, "Comparaison avec la calculette");
