...
$year = ConfigurationClient::getInstance()->getCampagneVinicole()->getCurrentYearPeriode();
...
$jsonContentTemplate = file_get_contents(dirname(__FILE__).'/../data/dr_douane.json');

$jsonTmpFile = tempnam(sys_get_temp_dir(), 'openodg');
file_put_contents($jsonTmpFile, str_replace(array("%campagne%", "%cvi%", "%code_inao_1%", "%libelle_produit_1%","%code_inao_2%", "%libelle_produit_2%"), array($campagne, "7523700100", $produit1->getCodeDouane(), $produit1->getLibelleComplet(), $produit2->getCodeDouane(), $produit2->getLibelleComplet()), $jsonContentTemplate));

$json = new DRDouaneJsonFile($jsonTmpFile);
$csvConvert = $json->convert();
unlink($jsonTmpFile);
...
$t->is($line[DRCsvFile::CSV_RECOLTANT_CVI], '"7523700100"', "Le CVI est 7523700100 en chaine de caractère");
