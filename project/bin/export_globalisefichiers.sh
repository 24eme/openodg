...
if ! test -f $(echo $0 | sed 's/[^\/]*$//')config.inc && ! test $1 ; then
    ...
    tail -n +2 $EXPORTGLOBALDIR/production.csv.drev.part $EXPORTGLOBALDIR/production.csv.sansdrev.part | grep -va ^== | grep -a ';' | iconv -f ISO88591 -t UTF8 | awk -F ';' '{uniq = $1"-"$2"-"$4 ; if ( ! unicite[uniq] || unicite[uniq] == $3 ) { print $0  ; unicite[uniq] = $3 } }'  >> $EXPORTGLOBALDIR/production.csv.part
