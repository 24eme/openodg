...
var timeout = 15000;
var nightmare = Nightmare({ show: false, typeInterval: 1, waitTimeout: timeout, gotoTimeout: timeout, executionTimeout: timeout, timeoutDownloadBeforeStart: timeout, maxDownloadRequestWait: timeout, webPreferences: { preload: path.resolve("pre.js") }});
...
nightmare
 ...
 .then(function() {
  ...
  .then(function() {
      ...
      var uri = baseUri+"/operateur/SynHabR.aspx";
      var exportFilename = destination_file+'01_operateurs/synthese_habilitations.html';
      console.log("export " + uri + ": " + exportFilename);
  ...
  .then(async function() {
    if(!regroupement) {
      ...
      var uri = baseUri+"/Declaration/SyntheseRecolteR.aspx";
       await nightmare
        .goto(uri)
        .wait('body')
        .exists("#ddlAnnee")
  ...
  .then(async function() {
    if(!regroupement) {
      ...
      var uri = baseUri+"/Declaration/SyntheseChangDenR.aspx";
 ...
 .then(function() {
  ...
  .then(function() {
    if(!regroupement) {
      ...
      var uri = baseUri+"/commission/SuiviCommission_med.aspx";
      var exportFilename = destination_file+'04_controles_produits/commissions_prevues.html';
      console.log("export " + uri + ": " + exportFilename);
